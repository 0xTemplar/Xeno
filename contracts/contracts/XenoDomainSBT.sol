...
import {IXenoDomainSBT} from "./interface/IXenoDomainSBT.sol";
...
contract XenoDomainSBT is IXenoDomainSBT, ERC4973, Ownable, ReentrancyGuard {
    ...
    function tokenURI(
        ...
        return
            IXenoDomainMetadata(metadataAddress).getMetadata(
    ...
    function burn(string calldata _domainName) external {
        ...
        require(
            domains[dName].holder == _msgSender() || _msgSender() == owner(),
        ...
        if (
            keccak256(bytes(defaultNames[_msgSender()])) ==
    ...
    function editData(
        ...
        string memory dName = strings.lower(_domainName);
        require(
            domains[dName].holder == _msgSender(),
            "Only domain holder can edit"
        );
    ...
    function mint(
        ...
        require(
            buyingEnabled || _msgSender() == owner() || _msgSender() == minter,
            "Buying domains disabled"
        );
    ...
    function _mintDomain(
        ...
        require(
            strings.len(strings.toSlice(_domainName)) > 0,
            "Domain name empty"
        );
        require(
            bytes(_domainName).length < nameMaxLength,
            "Domain name is too long"
        );
        require(
            strings.count(strings.toSlice(_domainName), strings.toSlice(".")) ==
                0,
        ...
        require(
            domains[_domainName].holder == address(0),
        ...
        emit DomainCreated(
            _msgSender(),
            _domainHolder,
            string(abi.encodePacked(_domainName, name()))
        );
    ...
    function _sendPayment(uint _amount) internal {
        (address addr, uint percentage) = IXenoDomainFactory(factoryAddress)
            .getRoyaltyDetails();

        if (percentage > 0 && percentage < 50) {
            // send royalty - must be less than 50%
            (bool sentRoyalty, ) = payable(addr).call{
                value: ((_amount * percentage) / 100)
            }("");
    ...
    function changeMetadataAddress(
        address _metadataAddress
    ) external onlyOwner {
